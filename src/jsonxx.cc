...
#include <cassert>
void
jsonxx::assertion(const char * file, int line, const char * expression, bool result)
{
  if (!result)
  {
...
bool
match(const char * pattern, std::istream & input);
bool
parse_array(std::istream & input, Array & array);
bool
parse_bool(std::istream & input, Boolean & value);
bool
...
bool
parse_string(std::istream & input, String & value);
bool
parse_identifier(std::istream & input, String & value);
bool
parse_value(std::istream & input, Value & value);
...
bool
Value::parse(std::istream & input, Value & value)
{
  value.reset();
...
static std::ostream &
stream_string(std::ostream & stream, const std::string & string)
{
  stream << '"';
...
std::ostream &
operator<<(std::ostream & stream, const jsonxx::Value & v)
{
...
std::string
remove_last_comma(const std::string & _input)
{
...
std::string
close_tag(unsigned format, char type, const std::string & name)
{
  switch (format)
  {
    default:
...
bool
validate(const std::string & input)
{
...
std::string
reformat(const std::string & input)
{
  std::istringstream is(input);
...
std::string
xml(std::istream & input, unsigned format)
{
  using namespace xml;
  JSONXX_ASSERT(format == jsonxx::JSONx || format == jsonxx::JXML || format == jsonxx::JXMLex ||
                format == jsonxx::TaggedXML);
...
std::string
xml(const std::string & input, unsigned format)
{
  std::istringstream is(input);
...
void
Object::import(const std::string & key, const Value & value)
{
  odd.clear();
...
std::string
Object::write(unsigned format) const
{
...
void
Array::import(const Value & value)
{
...
bool
Array::parse(const std::string & input)
{
  std::istringstream is(input);
  return parse(is, *this);
}
